...
describe('FieldSelector', () => {
    it('should display additional class names', () => {
        render(<FieldSelector data-testid="field-selector" className="extra" selector={<textarea placeholder="Input value"/>}/>);
    ...
    it('should display the button', () => {
        render(<FieldSelector buttons={<Button label="Click me"/>} selector={<textarea value="Input value"/>}/>);
        expect(screen.queryByText('Click me')).toBeInTheDocument();
    });

    it('should display multiple buttons', () => {
        render(<FieldSelector buttons={<><Button icon={<Add/>} label="Click me"/><Button icon={<Love/>} label="Click me"/></>} selector={<textarea value="Input value"/>}/>);
        expect(screen.queryAllByText('Click me')).toHaveLength(2);
    });

    it('should display the dragIcon', () => {
        const {container} = render(<FieldSelector isDraggable selector={<textarea placeholder="Input value"/>}/>);
        expect(container.querySelector('div.moonstone-cardSelector_dragIcon')).toBeInTheDocument();
    });
